...
import org.apache.spark.mllib.tree.impurity.{Variance4GBDT => OldVariance}
...
private[spark] object GradientBoostedTrees extends Logging {
  ...
  def computeInitialPredictionAndError(
      ...
      loss: OldLoss,
      initBias: Double = 0.0): RDD[(Double, Double)] = {
    data.map { lp =>
      val pred = updatePrediction(lp.features, initBias, initTree, initTreeWeight)
  ...
  def boost(
    ...
    logError("input.count:" + input.count())
    logError("labelBias:" + labelBias)
    logError("gradient for label 1:" + (-loss.gradient(labelBias, 1)))
    logError("gradient for label -1:" + (-loss.gradient(labelBias, -1)))
    val initInput = input.map(oldLabelPoint =>
      LabeledPoint(-loss.gradient(labelBias, oldLabelPoint.label), oldLabelPoint.features))
    val firstTree = new DecisionTreeRegressor().setSeed(seed)
    val firstTreeModel = firstTree.train(initInput, treeStrategy)
    val firstTreeWeight = 1.0
    baseLearners(0) = firstTreeModel
    baseLearnerWeights(0) = firstTreeWeight

    var predError: RDD[(Double, Double)] =
      computeInitialPredictionAndError(input, firstTreeWeight, firstTreeModel, loss, labelBias)
